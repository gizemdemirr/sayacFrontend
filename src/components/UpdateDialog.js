      ...
      <Dialog.Container visible={open}>
        ...
        <Dialog.Description>
          <View>
            <CustomInput
              label={'Abone No'}
              value={homeTeam.subNo ? homeTeam.subNo.toString() : ''}
              setValue={text => {
            ...
            <CustomInput
              label="Sayac No"
              value={homeTeam.subCountNo ? homeTeam.subCountNo.toString() : ''}
              setValue={text => {
                setHomeTeam({...homeTeam, subCountNo: text});
              }}
            />
            <CustomInput
              label="Harcanan Metre Küp"
              value={
                homeTeam.spentMetreKup ? homeTeam.spentMetreKup.toString() : ''
              }
              setValue={text => {
        ...
        <Dialog.Button
          ...
          onPress={async () => {
            await axios.put(
              'http://10.110.213.66:9090/posts/' + `${homeTeam?.id}`,
              homeTeam,
            );
          }}
        />
