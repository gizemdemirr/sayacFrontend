...
const Home = () => {
  ...
  const listSend = async () => {
    await AsyncStorage.getItem('user').then(async res => {
      ...
      const responses = await axios.post(
        'http://10.110.213.34:9090/posts/addCount',
        deneme,
      );
  ...
  useEffect(() => {
    AsyncStorage.getItem('userId').then(respo => {
      ...
      axios
        .get(
          'http://10.110.213.34:9090/posts/getUserIdCount?userId=' + `${value}`,
        )
        .then(res => {
          setData(res.data);
        });
  ...
  return (
    <View style={styles.container}>
        ...
        <Dialog.Description>
          <View>
            <CustomInput
              ...
              value={homeTeam.subNo}
              setValue={text => {
                setHomeTeam({...homeTeam, subNo: text});
              }}
            />
            <CustomInput
              placeholder="Sayac Model"
              value={homeTeam.countModel}
              setValue={text => {
                setHomeTeam({...homeTeam, countModel: text});
              }}
            />
            <CustomInput
              placeholder="Sayac Marka"
              value={homeTeam.countMark}
              setValue={text => {
                setHomeTeam({...homeTeam, countMark: text});
              }}
            />
            <CustomInput
              placeholder="Sayac No"
              value={homeTeam.subCountNo}
              setValue={text => {
                setHomeTeam({...homeTeam, subCountNo: text});
              }}
            />
            <CustomInput
              placeholder="Harcanan Metre Küp"
              value={homeTeam.spentMetreKup}
              setValue={text => {
      ...
      <DataTable>
        <DataTable.Header>
          <DataTable.Title style={styles.title} numeric>
            User ID
          </DataTable.Title>
          ...
          return (
            <Pressable
              ...
              onPress={() => {
                openDialog();
                setSelectedData({
                  subCountNo: item.subCountNo,
                  spentMetreKup: item.spentMetreKup,
                  metreKup: '5.56',
                });
              }}
              key={item.id}>
                  ...
                  <View style={styles.actionCell}>
                    <IconButton
                      ...
                      onPress={async () => {
                        try {
                          await axios.delete(
                            'http://10.110.213.34:9090/posts/' + `${item.id}`,
                          );
                        } catch (error) {
                          console.warn(error);
                        }
      ...
      <CustomDialog
        disabled={true}
        subCountNo={selectedData.subCountNo}
        spentMetreKup={selectedData.spentMetreKup}
        metreKup={selectedData.metreKup}
        open={open}
        handleClose={() => {
          setOpen(false);
        }}
      />
      <UpdateDiyalog
        homeTeam={selectUpdate}
        open={show}
        handleClose={handleClose}
        setHomeTeam={setSelectUpdate}
      />
    </View>
  );
};
